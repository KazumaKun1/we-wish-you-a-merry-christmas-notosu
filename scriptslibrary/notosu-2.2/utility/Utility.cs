...
namespace StorybrewScripts
{
    ...
    {
        public static Vector2 GetPointOnCircumference(Vector2 center, float r, float radians)
        {
            // Calculate the coordinates using trigonometric functions
            float x = center.X + r * (float)Math.Cos(radians);
            float y = center.Y + r * (float)Math.Sin(radians);
