...
namespace StorybrewScripts
{
    public static class OriginToReceptor
    {
        ...
        public static string drawNotesByOriginToReceptor(DrawInstance instance, double duration,  bool enableAdditive, bool renderReceptor = true)
        {
            ...
            Parallel.ForEach(playfieldInstance.columns.Values, column =>
            {
                ...
                Parallel.ForEach(keysInRange, key =>
                {
                    ...
                    if (enableAdditive) {
                        note.Additive(renderStartTime, renderStartTime);
                    }
                    ...
                    do
                    {
                        ...
                        if (noteFade != null)
                        {
                            var actualFadeInTime = noteFade.starttime > renderStartTime ? noteFade.starttime : renderStartTime;
                            var actualFadeOutTime = noteFade.endtime > renderEndTime ? renderEndTime : noteFade.endtime;

                            note.Fade(actualFadeInTime, actualFadeOutTime, noteFade.easing, noteFade.value);
                        }
                    ...
                    Parallel.ForEach(note.sliderPositions, part =>
                         {
                             ...
                             if (sliderFade != null)
                             {
                                var actualFadeInTime = sliderFade.starttime > sliderRenderStartTime ? sliderFade.starttime : sliderRenderStartTime;
                                var actualFadeOutTime = sliderFade.endtime > sliderRenderEndTime ? sliderRenderEndTime : sliderFade.endtime;

                                sprite.Fade(actualFadeInTime, sliderFade.value);
                             }
                             else
                             {
                             ...
                             do
                             {
                                 ...
                                 if (sliderFade != null)
                                 {
                                     if (sprite.OpacityAt(sliderCurrentTime) != sliderFade.value) {
                                        var actualFadeInTime = sliderFade.starttime > sliderCurrentTime ? sliderFade.starttime : sliderCurrentTime;
                                        var actualFadeOutTime = sliderFade.endtime > sliderRenderEndTime ? sliderRenderEndTime : sliderFade.endtime;

                                        sprite.Fade(sliderFade.easing, actualFadeInTime, actualFadeOutTime, sprite.OpacityAt(sliderCurrentTime), sliderFade.value);
                                     }
