...
namespace StorybrewScripts
{
    ...
    public class Playfield : IDisposable
    {
        ...
        public void initilizePlayField(StoryboardLayer receptors, StoryboardLayer notes, double starttime, double endtime, float initialWidht, float initialHeight, float receptorWallOffset, bool enableAdditive = false)
        {
            ...
            if (enableAdditive) {
                receptorSpritePath = "sb/sprites/receiver-a.png";
            }

            Column one = new Column(128, ColumnType.one, receptorSpritePath, receptors, receptorScale, starttime, delta, enableAdditive);
            Column two = new Column(256, ColumnType.two, receptorSpritePath, receptors, receptorScale, starttime, delta, enableAdditive);
            Column three = new Column(384, ColumnType.three, receptorSpritePath, receptors, receptorScale, starttime, delta, enableAdditive);
            Column four = new Column(512, ColumnType.four, receptorSpritePath, receptors, receptorScale, starttime, delta, enableAdditive);
        ...
        public void initializeNotes(List<OsuHitObject> objects, double bpm, double offset, bool enableAdditive = false, bool isColored = false, double msPerPart = 30)
        {
            ...
            foreach (Column column in columns.Values)
            {
                ...
                foreach (OsuHitObject hitobject in objects)
                {
                    ...
                    {
                        Note currentNote = new Note(this.noteLayer, hitobject, column, bpm, offset, enableAdditive, isColored, msPerPart);
        ...
        public void Resize(OsbEasing easing, double starttime, double endtime, float width, float height, bool bypassZeroLimit = false)
        {
        ...
        */
        ...
        public void RotatePlayFieldStatic(OsbEasing easing, double starttime, double endtime, double radians, ColumnType columnType = ColumnType.all)
        {
        ...
        public Vector2 GetReceptorPositionOf(ColumnType columnType, int currentTime) {
            Column column = columns[columnType];

            Vector2 position = column.ReceptorPositionAt(currentTime);
